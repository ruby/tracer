...
module EnvUtil
  def rubybin
    ...
    defined?(RbConfig.ruby) ? RbConfig.ruby : "ruby"
  end
  ...
  class << self
    ...
    attr_reader :original_internal_encoding,
    ...
    def capture_global_values
      ...
      @original_warning =
        (
          if defined?(Warning.[])
            %i[deprecated experimental].to_h { |i| [i, Warning[i]] }
          else
            nil
          end
        )
  ...
  def terminate(pid, signal = :TERM, pgroup = nil, reprieve = 1)
    ...
    signals =
      Array(signal).select do |sig|
        begin
          DEFAULT_SIGNALS[sig.to_s] or DEFAULT_SIGNALS[Signal.signame(sig)]
    ...
    while signal = signals.shift
      if lldb and %i[ABRT KILL].include?(signal)
        lldb = false
  ...
  def invoke_ruby(
    ...
    **opt
    ...
    err_p, err_c = IO.pipe if capture_stderr &&
      capture_stderr != :merge_to_stdout
    opt[:in] = in_c
    opt[:out] = out_c if capture_stdout
    opt[:err] = (
      if capture_stderr == :merge_to_stdout
        out_c
      else
        err_c
      end
    ...
    child_env.update(args.shift) if Array === args and Hash === args.first
    if RUBYLIB and lib = child_env["RUBYLIB"]
      child_env["RUBYLIB"] = [lib, RUBYLIB].join(File::PATH_SEPARATOR)
    end

    # remain env
    %w[ASAN_OPTIONS RUBY_ON_BUG].each do |name|
    ...
    if block_given?
      ...
      in_p.close
      if (!th_stdout || th_stdout.join(timeout)) &&
           (!th_stderr || th_stderr.join(timeout))
        timeout_error = nil
      else
    ...
    [th_stdout, th_stderr].each { |th| th.kill if th }
    [in_c, in_p, out_c, out_p, err_c, err_p].each { |io| io&.close }
    [th_stdout, th_stderr].each { |th| th.join if th }
  end
  ...
  def with_default_external(enc)
    ...
    suppress_warning do
      Encoding.default_external = EnvUtil.original_external_encoding
    end
  end
  ...
  def with_default_internal(enc)
    ...
    suppress_warning do
      Encoding.default_internal = EnvUtil.original_internal_encoding
    end
  end
  ...
  if /darwin/ =~ RUBY_PLATFORM
    ...
    def self.diagnostic_reports(signame, pid, now)
      ...
      cmd = @ruby_install_name if "ruby-runner#{RbConfig::CONFIG["EXEEXT"]}" ==
        cmd
  ...
  def self.failure_description(status, now, message = "", out = "")
    ...
    sigdesc = "SIG#{signame} (#{sigdesc})" if signame
    sigdesc = "#{sigdesc} (core dumped)" if status.coredump?
    full_message = "".dup
    ...
    unless defined?(@gc_stress_to_class)
      _, _, status =
        invoke_ruby(
          [
            "-e" \
              "exit GC.respond_to?(:add_stress_to_class)"
          ]
        )
